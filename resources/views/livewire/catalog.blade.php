<div class="max-w-7xl mx-auto px-6 pt-28 pb-16">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-8 bg-[#0F0F0F] p-4 rounded-xl border border-white/5">
        ...
        <div class="flex-1 relative">
            <input wire:model.live.debounce.300ms="search"
                   ...
                   class="w-full p-3 rounded-lg bg-black/50 text-white border border-transparent focus:border-accent focus:ring-0 transition">

            {{-- Loading Spinner --}}
            <div class="absolute right-3 top-3.5">
                <svg wire:loading wire:target="search" class="animate-spin h-5 w-5 text-accent"
    ...
    <div class="flex border-b border-white/10 mb-8">
        @foreach(['Anime', 'Manga', 'All'] as $tab)
            <button wire:click="$set('type', '{{ $tab }}')"
                    class="px-6 py-3 text-lg font-bold transition duration-300 border-b-2 {{ $type === $tab ? 'border-accent text-accent' : 'border-transparent text-gray-400 hover:text-white hover:border-white/20' }}">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-6">
        ...
        @empty
            <div class="col-span-full text-center py-20 bg-[#0F0F0F] rounded-xl border border-white/5">
                ...
                <button wire:click="$set('search', '')" class="text-accent hover:underline mt-2 inline-block">Clear
                    Search
                </button>
            </div>
        @endforelse
